...
import '../sdk-helper/sdk_request_handler.dart';
import '../utils/extension.dart';
import 'txn_history.dart';
import 'widget/progress_indicator.dart';

class StartTxn extends StatefulWidget {
  const StartTxn({super.key});

  @override
  State<StartTxn> createState() => _StartTxnState();
}

class _StartTxnState extends State<StartTxn> {
  late String traceId;
  late String orderId;
  ...
  processPayment() async {
    ...
    //order creation
    var response = await ApiRequestHandler(
      traceId: traceId,
      timeStamp: timeStamp,
      orderDate: orderDate,
      orderId: orderId,
    ...
    if (response.statusCode == 200) {
      ...
      debugPrint("Create Order Response for order creation :$createOrderRes");
      SDKRequestHandler().launcherSDK(createOrderRes);
      setState(() {
        isPayEnabled = false;
      });
    } else {
